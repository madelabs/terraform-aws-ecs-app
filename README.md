...
| <a name="input_container_secrets"></a> [container\_secrets](#input\_container\_secrets) | The secrets from AWS Secrets Manager to pass to a container. | <pre>list(object({<br>    name      = string,<br>    valueFrom = string<br>  }))</pre> | `[]` | no |
